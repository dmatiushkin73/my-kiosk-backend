    ...
    def do_POST(self):
        ...
        if content_length > 0:
            ...
            if self.server._logger:
                ...
                print(log_msg)
            self.send_response(400, 'Content-length is 0 or absent')
            self.end_headers()
...
class MyHTTPServer(HTTPServer):
    ...
    def on_get(self, func):
        """Define the get callback.
           Expected signature get_callback(path, headers, body)->tuple(status_code, headers, body)
        """
        self._on_get = func
    ...
    def on_post(self, func):
        """Define the post callback.
           Expected signature post_callback(path, headers, body)->tuple(status_code, headers, body)
        """
        self._on_post = func
    ...
    def on_put(self, func):
        """Define the put callback.
           Expected signature put_callback(path, headers, body)->tuple(status_code, headers, body)
        """
        self._on_put = func
...
class RestServerBase(AppModule):
    ...
    @abstractmethod
    def _on_get(self, path: str, headers: dict, msg: str) -> tuple[int, dict, str]:
        return 501, {}, ''

    @abstractmethod
    def _on_post(self, path: str, headers: dict, msg: str) -> tuple[int, dict, str]:
        return 501, {}, ''

    @abstractmethod
    def _on_put(self, path: str, headers: dict, msg: str) -> tuple[int, dict, str]:
        return 501, {}, ''
